...
def read(book, ind_book):
    ...
    for occur in word_frequencies(txt):
        index[ind_book][words[occur[1]]] = occur[0]

    print("fill book %d / %d" % (ind_book, len(books)) )
...
def read_words(book):
    ...
    for _, word in word_frequencies(txt):
...
if __name__ == "__main__":
    ...
    for i in range(len(books)):
        # read the book
        f = open(book_dir + "/" + books[i], "r")
        txt = ' '.join(f.readlines())
        words_of_book.append([occ for occ in word_frequencies(txt)])
        print("Book %d / %d" % (i, len(books)))
    ...
    index = np.zeros((len(books), len(words.keys())), dtype=np.int64)

    t_read = time.time()
    for i in range(len(books)):
        read(books[i], i)
    print("** Time reading : %.3f seconds" % (time.time() - t_read))

    words_json = {}
    for w in list(words.keys())[:5]:
        for i in range(len(index)):
            words_json[w] = [int(index[i][words[w]]) for i in range(len(index))]
    ...
    """for i in range(len(index)):
        print([index[i][j] for j in range(len(index[i]))])
        print("------------")"""
